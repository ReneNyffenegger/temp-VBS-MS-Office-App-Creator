...
function getActiveVBProject(app) ' {
    ...
    if app.name = "Microsoft Word" then
       ...
       set getActiveVBProject = vb_editor.activeVBProject
    end if
...
'   dim vb_editor ' as vbe
    ...
    set vb_proj = getActiveVBProject(app)
'   if app.name = "Microsoft Word" then
'      set vb_proj   = app.activeDocument.vbProject
'   else
...
sub passwordProtect(app, password) ' {
  '
  ' Security theater: basic password protection for
  ' the code, but can easily be bypassed
  '
  ' https://stackoverflow.com/a/53323958/180275
  '
    dim vb_proj
    set vb_proj = getActiveVBProject(app)

    if vb_proj.protection = 1 then ' {
       wscript.echo "The VBA code is already protected"
       exit sub
    end if ' }
  ...
  '
  ' Simulate click on Visual Basic Editor Menu: Tools -> Xyz Project Properties
  '
    app.vbe.commandBars(1).findControl(1, 2578, "", true, true).execute ' 1 = msoControlButton, 2578: the ID of the controk 

  '
  ' With the 'Project Properties' message box now open, we use sendKeys to
  ' protect and enter the password
  '
    app.sendKeys "^{tab}"      ' ctrl-tab: go to 'Protection' tab
    app.sendKeys  "{tab}"      ' advance to 'Lock project for viewing' check box
    app.sendKeys  " "          ' check checkbox (code ASSUMES the checkbox is not checked)
    app.sendKeys "{tab}{tab}"  ' Advance to first password field (apparantly, two tabs are needed) ....
    app.sendKeys password      ' ... and enter password
    app.sendKeys "{tab}"       ' Advance to 'confirm password field ...
    app.sendKeys password      ' ... and re-enter password
    app.sendKeys "{tab}"
    app.sendKeys "{enter}"
